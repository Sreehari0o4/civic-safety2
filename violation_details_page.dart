  ...
  Future<void> _fetchDetails() async {
    ...
    try {
      ...
      if (imageId.isNotEmpty) {
        ...
        final projectId =
            widget.client.config['project']; // Appwrite project ID
        final bucketId = '67c3ece6001c2b68828b'; // Your bucket ID

        // Construct the image URL manually
        final fileViewUrl =
            '$endpoint/storage/buckets/$bucketId/files/$imageId/view?project=$projectId';
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: () => approveViolation(
                            violation.$id, violation.data['user_id']),
                        child: Text("Approve"),
                      ),
                      ElevatedButton(
            ...
            TextButton(
              onPressed: () async {
                if (fineController.text.isNotEmpty) {
                  try {
                    int fineAmount =
                        (double.parse(fineController.text) * 100).toInt();
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Violation Approved & ₹${rewardAmount / 100} reward added')),
                    );
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
              "Are you sure you want to block this user? All their submitted violations will be cancelled."),
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(context), child: Text('Cancel')),
            TextButton(
              onPressed: () async {
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text('User Blocked & Violations Cancelled')),
                  );
